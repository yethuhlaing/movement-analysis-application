...
class App(tk.Tk):
    ...
    def show_second_page(self, project_name,project_creator, scenario, reference, student):
        if self.second_page:
            self.second_page.destroy()
        self.first_page.pack_forget()
        self.second_page = ProjectCreation(self, project_name,project_creator, scenario, reference, student)
