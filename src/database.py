...
def insertHistory(database):
    project = getHeadingData()["project_name"]
    scenario = getVisualizationData()["scenario"]
    student = getInformationData()["student_name"]

    reference_df_list = serializeDataframeList(getReference_df())
    student_df_list = serializeDataframeList(getStudent_df())
    status_df_list = serializeDataframeList(getStatus_df())
    userData = serialize(getUserData())
...
def selectStudentID(cursor, project, scenario, student):
    print(project, scenario, student)
    cursor.execute('''
        SELECT student_id 
        FROM student
        WHERE project = ? AND
