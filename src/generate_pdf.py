...
def analyze(user_data: list):
    ...
    c.drawString(52, HEIGHT - 340, "Project - {}".format(headingData["project_name"]))
    c.drawString(52, HEIGHT - 360,  "Creator - {}".format(headingData["project_creator"]))
    c.drawString(52, HEIGHT - 380, "Scenario - {}".format(visualizationData["scenario"]))
    c.drawString(52, HEIGHT - 400, "Student - {}".format(informationData["student_name"]))
    ...
    for category, movementArray in visualizationData["categories"].items():
        for index, movement in enumerate(movementArray):
            _, _, duration,total_frames, starting_time,frame_rate, Graph_type, ref_name, ref_file, student_name, student_file = visualizationData.values()
